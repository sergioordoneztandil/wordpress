...
while ( $office->have_posts() ) : $office->the_post();
	...
	$offices[get_the_ID()] = $office_location;
endwhile;
...
function advisor_meta_boxes( $meta_boxes ) {
	...
	$meta_boxes[] = array(
        ...
        'fields'     => array(
			...
			array(
				'id'   => 'advisor_member_office',
				'name' => esc_html__( 'Office', 'advisor' ),
				'type' => 'Select',
				'options' => $offices,
			),
			array(
	...
	$meta_boxes[] = array(
		'title'      => esc_html__( 'Office', 'advisor' ),
		'post_types' => 'office',
		'fields'     => array(
			array(
					'id'         => 'office_type',
					'name'       => esc_html__( 'Type', 'advisor' ),
					'type'       => 'taxonomy',
					'taxonomy'   =>  'office-type', /*'case-study-type'*/
					'field_type' => 'select',
					'multiple'   => false
			),
			array(
				'id'   => 'office_location',
				'name' => esc_html__( 'Location', 'advisor' ),
				'type' => 'text',
			),
			array(
				'id'   => 'office_detail',
				'name' => esc_html__( 'Office detail', 'advisor' ),
				'type' => 'WYSIWYG',
			),
		),
	);
